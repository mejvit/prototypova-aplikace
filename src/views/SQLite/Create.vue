  ...
  <ion-page>
    <ion-header :translucent="true">
      <ion-toolbar>
        ...
        <ion-title>Nový záznam</ion-title>
      </ion-toolbar>
    </ion-header>
    <ion-content v-if="hybridPlatform">
      ...
      <ion-button expand="block" @click="saveRecord()">
...
export default defineComponent({
  ...
  setup() {
    ...
    const saveRecord = async function () {
      if (db != null) {
        const sqlcmd =  "INSERT INTO records (name,number,date,description) VALUES (?,?,?,?)";
        const values: Array<any>  = [record.value.name, record.value?.number, record.value?.date, record.value?.description];
